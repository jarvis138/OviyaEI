...
def test_2_audio_drift():
    ...
    for i, (text, emotion, intensity) in enumerate(test_cases, 1):
        ...
        try:
            ...
            if drift_percent <= 30:  # Allow 30% drift (more lenient)
                ...
                print(f"   ⚠️  High drift (but may be normal for emotional speech)")
    ...
    if avg_drift <= 30:
        ...
        print("⚠️  WARNING: High audio drift (may be normal for emotional speech)")
...
def test_3_emotion_distribution():
    ...
    if max_percent < 50:
        ...
        print("⚠️  WARNING: High concentration in one emotion")
...
def test_4_stability_and_fallbacks():
    ...
    if response.get('emotional_state'):
        ...
        if state['energy_level'] > 0.3 and state['warmth'] > 0.5:
            ...
            print("   ⚠️  Default state may be too low-energy")
    ...
    try:
        ...
        if params['emotion_label'] == 'neutral':
            ...
            print(f"   ⚠️  Falls back to: {params['emotion_label']}")
            test2_pass = True
    except Exception as e:
        print(f"   ❌ Error: {e}")
        test2_pass = False
    ...
    if all_pass:
        ...
        print("\n⚠️  WARNING: Some fallback issues detected")
        return False
...
def test_5_performance_metrics():
    ...
    if avg_latency <= 5.0:
        ...
        print(f"   ⚠️  High latency ({avg_latency:.1f}s)")
        return False
...
def main():
    ...
    if passed == total:
        ...
        print("\n⚠️  MOSTLY READY: Most tests passed, review failures")
        return 0
    else:
